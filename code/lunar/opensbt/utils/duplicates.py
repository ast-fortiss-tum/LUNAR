...
def duplicate_free(population, is_equal=default_is_equal):
    ...
    # TODO: define properly for conversations
    elif len(inds) > 0 and isinstance(inds[0][0], Conversation):
        inds_str = np.asarray([np.asarray([ind[0].get_dialogue_history_str()]) for ind in inds])
        dup_free = [population[i] for i in remove_duplicates(inds_str, is_equal)]
    else:
