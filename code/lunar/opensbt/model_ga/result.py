...
class SimulationResult(Result):
    ...
    def write_results(
        ...
        try:
            # Make sure logging happens even when sth fails.
            ...
            llm_output.write_tests_to_json_compact(self, save_folder)
            llm_output.write_tests_to_json_compact(
                self, save_folder, critical_only=True, filename="all_critical_utterances"
            )
            ...
            llm_output.write_novelty_archive_to_json(self, save_folder)
            visualizer.backup_problem(self, save_folder)
            # llm_output.write_simout_to_json(self, save_folder)

        except Exception as e:
            traceback.print_exc()
            print("Error in write results ocurred.")
