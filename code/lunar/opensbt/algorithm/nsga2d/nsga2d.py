...
class NSGA2D(GeneticAlgorithm):
    ...
    def _advance(self, infills=None, **kwargs):
        ...
        if self.n_repopulate_max > 0:
            ...
            # need to repair, as default sampling just produces vars with no concrete inds based on use case
            pop_sampled = self.repair(self.problem, pop_sampled, **kwargs)
            ...
            # use actual number of sampled individuals (may be less than requested)
            n_actual_repopulate = min(n_repopulate, len(pop_sampled))
            
            for i in range(1, n_actual_repopulate + 1):
