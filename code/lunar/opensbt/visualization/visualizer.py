...
from llm.model.models import Conversation, Utterance
...
def write_summary_results(res, save_folder, params):
    ...
    log.info(["Tokens Used", ModelStatistics.total_values()["total_tokens"]])
    log.info(
        ["Costs Approximated ($)", ModelStatistics.total_values()["costs"]["total"]]
    )
...
def all_individuals(res, save_folder):
    ...
    with open(save_folder + "all_testcases.csv", "w", encoding="UTF8", newline="") as f:
        ...
        for index, ind in enumerate(all_individuals):
            row = [index]
            if isinstance(ind.get("X")[0], Conversation):
                row.extend([f"%s" % X_i.ordinal_vars for X_i in ind.get("X")])
                row.extend([f"%s" % X_i.get_dialogue_history_str() for X_i in ind.get("X")])
    ...
    for i, algo in enumerate(hist):
        with open(
            ...
            for i in range(len(algo.pop)):
                row = [index]
                if isinstance(algo.pop.get("X")[i][0], Conversation):
                    row.extend([f"%s" % X_i.ordinal_vars for X_i in algo.pop.get("X")[i]])
                    row.extend([f"%s" % X_i.get_dialogue_history_str() for X_i in algo.pop.get("X")[i]])
                    # row.extend([f"%s" % X_i.question for X_i in ind.get("X")])
                    row.extend([f"%s" % F_i for F_i in algo.pop.get("F")[i]])
                elif isinstance(algo.pop.get("X")[i][0], Utterance):
