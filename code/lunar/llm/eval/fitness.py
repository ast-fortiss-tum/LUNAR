...
from llm.adapter.embeddings_openai_adapter import get_similarity_individual, get_disimilarity_individual, get_disimilarity_conversation
...
class FitnessDiverseMultiTurn(Fitness):
    def __init__(self, diversify=False) -> None:
        super().__init__()
    ...
    def eval(self, simout: MultiTurnSimulationOutput, **kwargs) -> Tuple[float]:  
        ...
        if "algorithm" in kwargs:
            ...
            if not hasattr(algorithm, 'archive_novelty'):
                ...
                _, distance_archive = algorithm.archive_novelty.closest_individual_from_vars(
                                                kwargs["individual"], 
                                                dist_fnc = get_disimilarity_conversation)
                print("Distance archive:", distance_archive)
        f_vector = (distance_archive,)
        return f_vector
...
class FitnessMerged(Fitness):
    ...
    def eval(self, simout: SimulationOutputBase, **kwargs) -> Tuple[float]:  
        ...
        # dont penalize when output missing and none exists
        if len(res) > 1 and isinstance(simout, QASimulationOutput):
            if res[1] == 1 and not simout.poi_exists:
                res = (1,1) + res[2:] # system response is fine
        return res
