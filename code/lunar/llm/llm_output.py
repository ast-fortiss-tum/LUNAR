...
from llm.model.models import Utterance, ContentInput, Conversation
from llm.model.qa_problem import QAProblem
...
class ExtendedEncoder(json.JSONEncoder):
    """
    Handles sets and numpy types for JSON serialization.
    """
...
class TestReportCompact(BaseModel):
...
def write_tests_to_json_compact(res, save_folder: str, critical_only: bool = False, filename: Optional[str] = None):
    ...
    if filename is None:
        filename = "critical_utterances_compact" if critical_only else "all_utterances_compact"
    ...
    for idx, ind in enumerate(all_population):
        conv: Conversation = ind.get("X")[0]  # Assuming "X" holds vars and is a list of lists
        ...
        conversation_compact = conv.get_dialogue_history_dict()
        intents = conv.get_intent_history_dict()
    ...
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(all_utterances, f, ensure_ascii=False, indent=4, cls=ExtendedEncoder)
...
def write_simout_to_json(res, save_folder: str, critical_only: bool = False, filename: Optional[str] = None):
    ...
    all_utterances: List[SimoutReport] = []
...
def write_tests_to_json(res, save_folder: str, critical_only: bool = False, filename: Optional[str] = None):
    ...
    for idx, ind in enumerate(all_population):
        utter: Conversation = ind.get("X")[0]  # Assuming "X" holds vars and is a list of lists
        # poi_exists = ind.get("SO").poi_exists
        # other = ind.get("SO").other
...
def write_novelty_archive_to_json(res, save_folder: str, filename = "novelty_archive"):
    ...
    if hasattr(algorithm, "archive_novelty"):
        ...
        problem = res.problem
        all_utterances: List[ArchiveTestReport] = []

        for idx, ind in enumerate(archive):
            # novelty archive can store Conversation (multi-turn) or Utterance (single-turn)
            conv_or_utter = ind.get("X")[0]
            poi_exists = getattr(ind.get("SO"), "poi_exists", None)

            fitness_names = list(problem.fitness_function.name)
            scores_dict = {
                name: float(score)
                for name, score in zip(fitness_names, ind.get("F").tolist())
            }
            is_critical = bool(ind.get("CB"))
            ...
            if hasattr(problem, "feature_handler"):
                ...
                # use getattr safely
                features_dict = feature_handler.get_feature_values_dict(
                    getattr(conv_or_utter, "ordinal_vars", None),
                    getattr(conv_or_utter, "categorical_vars", None),
                )
            else:
                features_dict = {}
            ...
            all_utterances.append(
                ArchiveTestReport(
                    ...
                    conversation=conv_or_utter,   # <-- FIX: was `utterance=...`
                    features_dict=features_dict,
                    fitness=scores_dict,
                    is_critical=is_critical,
                    # other = other,
