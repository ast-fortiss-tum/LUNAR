"""
Sample Navi feature configurations (content + style), applying the same content/style
constraints as `examples.navi.navi_utterance_generator.NaviUtteranceGenerator`,
but returning JSON-like dicts instead of generating utterance text.

Additionally, this sampler creates and writes N JSON files to the same folder
as this script (examples/navi/navi_sampler/). Each JSON contains:
- judge_dimensions: sampled scores in {0,1,2} for Clarity + Request-orientedness
- num_turns: sampled integer in [2, 8]
- content_input (constrained)
- style_input (constrained)
"""
...
JUDGE_DIMENSIONS: Dict[str, Any] = {
    ...
    "Request-orientedness": {
        ...
        "scores": {
            ...
            1: "System partially addresses the request; somewhat related but no concrete action offered, or asks for more info without having searched.",
            2: "System fully addresses the request; contains a relevant car function or clearly references the user's goal. A clear negative answer is acceptable if no suitable car function is available.",
        },
    },
}


class CCFeatureSampler:
    """
    Samples 1..N Car control instances: content_input + style_input (constrained).
    ...
    def __init__(
        ...
        self._gen = CCUtteranceGenerator(
    ...
    def _update_vars_from_model(
        ...
        # Mirror CCUtteranceGenerator._update_features_from_content_input()
...
def write_navi_samples_to_folder(
    n: int,
    features_config_path: str = "configs/features_simple_judge_cc.json",
    output_dir: str | Path | None = None,
    filename_prefix: str = "cc_sample_",
) -> List[Path]:
    """
    ...
    sampler = CCFeatureSampler(fhandler, apply_constrains_to_vars=True)
...
if __name__ == "__main__":
    ...
    paths = write_navi_samples_to_folder(
        n=30,
        features_config_path="configs/features_simple_judge_cc.json",
