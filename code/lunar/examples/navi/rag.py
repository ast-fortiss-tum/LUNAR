...
def get_shared_model(model_name: str, device: str = "cpu"):
    if model_name not in _shared_models:
        _shared_models[model_name] = SentenceTransformer(model_name, device=device)
...
class RAGRetriever:
    def __init__(
        ...
        self.embedding_model = get_shared_model(self.model_name, device=device)
