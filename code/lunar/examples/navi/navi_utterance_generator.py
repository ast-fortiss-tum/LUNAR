...
class NaviUtteranceGenerator(UtteranceGenerator):
    ...
    def apply_constraints(self, content_input: NaviContentInput) -> NaviContentInput:
        ...
        # Payment methods
        if category not in ["restaurant", "cafe", "hotel", "bar", "supermarket", "gas_station", "cinema", "bakery", "car_repair"]:
        ...
        # Parking
        if category not in ["hotel", "restaurant", "supermarket", "museum", "bar", "cafe", "hospital", "cinema", "bakery"]:
    ...
    def generate_utterance(
        ...
        llm_type: str,
        top_k_retrieved=5,
        content_input_override: Optional[NaviContentInput] = None,
    ) -> Utterance:
        ...
        try:
            ...
            print(f"[NaviUtteranceGenerator] Failed to generate question: {e}")
...
if __name__ == "__main__":
    fhandler = FeatureHandler.from_json("configs/features_simple_judge_all.json")
    set_seed(100)
    gen = NaviUtteranceGenerator(fhandler, use_rag = True)
    for i in range(40):
